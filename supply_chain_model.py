...
if __name__ == "__main__":
    ...
    fig2 = create_comparison_visualization(flows, cost, capacity, demand)
    plt.show()
    ...
    for i, f in enumerate(factories):
        for j, c in enumerate(customers):
            if flows[i, j] > 0:
                print(f"{f} → {c}: {flows[i, j]:.0f} units @ ${cost[i, j]}/unit")
    ...
    fuel_exposure = np.array([0.8, 1.2])
    cost_results = []

    for alpha in fuel_multipliers:
        adjusted_cost = cost.copy()

        for i in range(len(factories)):
            adjusted_cost[i, :] = cost[i, :] * (1 + (alpha - 1) * fuel_exposure[i])
    ...
    plt.title("Fuel Price Sensitivity Analysis")
    plt.show()  
